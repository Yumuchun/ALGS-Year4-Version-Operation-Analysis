...
A reproducible portfolio project analyzing Apex Legends version changes, ALGS pro-play meta shifts, and game operations monitoring metrics.

本项目是一份面向游戏运营 / 数据运营岗位的个人作品集，用 ALGS Year 4 四次国际赛事公开数据，分析 Apex Legends 版本更新如何影响职业赛场的角色、武器、阵容和打法选择，并将结论转化为版本运营监控指标与调优建议。

## 项目摘要

我想回答的问题是：**版本更新上线后，职业赛场到底发生了什么变化？**

项目选取 ALGS Year 4 四次国际赛事作为连续观察窗口，结合公开 patch notes、赛事得分、角色选择、阵容组合、武器统计和地图池数据，分析职业队在不同版本后的 Meta 响应。当前数据覆盖 **238 场比赛、4,760 条队伍得分记录、7,216 条武器统计**，并通过 Notebook 保留完整的数据校验、指标计算和图表生成流程。

核心结论是：Year 4 的版本更新确实改变了职业赛场，但变化并不主要体现在总得分结构上。真正剧烈变化的是角色、阵容和武器选择。职业赛场先后经历了 **AR 稳定输出集中 -> Akimbo / Shotgun 近战爆发集中 -> Support / 防守重置 体系**。

因此，本项目不只是赛后复盘，而是尝试建立一套版本上线后的运营监控框架：用击杀分占比、Top 5 率、PPG、阵容集中度、职业槽位占比、武器类别击杀占比和 reset 信号，判断新版本是否产生新的生态集中风险，并给出后续版本复盘与调优建议。

## 快速入口

- `report/`：完整作品集报告，适合直接阅读。
- `notebook/`：可复现数据分析 Notebook，包含数据校验、指标计算和图表生成。
- `data/`：公开赛事数据与处理后数据。
- `figures/report_figures/`：报告中使用的核心图表。
- `src/`：数据抓取、指标计算和报告图表生成代码。
